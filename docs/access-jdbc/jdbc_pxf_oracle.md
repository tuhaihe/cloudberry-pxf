...
description: Using PXF JDBC with Oracle.
sidebar_position: 4
---
...
For information about controlling parallel execution in Oracle, refer to [About Setting Parallel Query Session Parameters](#about-setting-oracle-parallel-query-session-parameters) located at the end of this topic.
...
2. Create a JDBC server configuration for Oracle as described in [Example Configuration Procedure](../administering/configuring/jdbc-connector/jdbc_cfg.md#example-configuration-procedure), naming the server directory `oracle`. The `jdbc-site.xml` file contents should look similar to the following (substitute your Oracle host system for `oracleserverhost`, and the value of your Oracle service name for `orcl`):
...
1. Create the PXF external table specifying the `jdbc` profile. For example:
	...
	OR create the PXF foreign table specifying `jdbc_pxf_fdw` Foreign Data Wrapper. For example:

	```sql
	gpadmin=# CREATE SERVER "oracle" FOREIGN DATA WRAPPER jdbc_pxf_fdw;
	gpadmin=# CREATE USER MAPPING FOR CURRENT_USER SERVER "oracle";
	gpadmin=# CREATE FOREIGN TABLE oracle_countries (country_id int, country_name varchar, population float)
				  SERVER "oracle"
				  OPTIONS (
...
1. Create a writable PXF external table specifying the `jdbc` profile. For example:
	...
	OR reuse PXF foreign table `oracle_countries` created in [Read from the Oracle Table](#read-from-the-oracle-table).
