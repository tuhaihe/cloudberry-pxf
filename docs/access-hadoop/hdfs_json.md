...
description: Reading JSON data in HDFS via PXF.
sidebar_position: 4
---
...
To represent JSON data in Apache Cloudberry, map data values that use a primitive data type to Apache Cloudberry columns of the same type. JSON supports complex data types including projections and arrays.
...
<sup>1</sup> PXF returns an error if Apache Cloudberry cannot convert the date or time string to the target type.
...
| PXF/Apache Cloudberry Data Type | JSON Data Type |
|-------------------|----------------------|
...
To specify the nested fields in the `user` object directly as Apache Cloudberry external table columns, use `.` projection:
...
To read all of the elements of the `values` array in a single column, define the corresponding Apache Cloudberry external table column as type `int[]`.
...
Use the `hdfs:json` profile to read or write JSON-format data in HDFS. The following syntax creates an Apache Cloudberry external table that references such a file:
...
| COMPRESSION_CODEC    | The compression codec alias. Supported compression codecs for writing json data include: `default`, `bzip2`, `gzip`, and `uncompressed`. If this option is not provided, Apache Cloudberry performs no data compression. |
| DISTRIBUTED BY    | If you are loading data from an existing Apache Cloudberry table into the writable external table, consider specifying the same distribution policy or `<column_name>` on both tables. Doing so will avoid extra motion of data between segments on the load operation. |
...
Once the data is loaded to HDFS, you can use Apache Cloudberry and PXF to query and add to the JSON data.

### Example: Single Object Per Row (Read)

Use the following [CREATE EXTERNAL TABLE](https://cloudberry.apache.org/docs/sql-stmts/create-external-table) SQL command to create a readable external table that references the single-object-per-row JSON data file and uses the PXF default server.
...
**Note**: When you use this method to identify specific array elements, PXF provides _only_ those values to Apache Cloudberry, not the whole JSON array.
...
PXF supports accessing all of the elements within an array as a single string containing the serialized JSON array by defining the corresponding Apache Cloudberry table column with one of the following data types: `TEXT`, `VARCHAR`, or `BPCHAR`.
...
If you retrieve the JSON array in a single text-type column and wish to convert the JSON array serialized as `TEXT` back into a native Apache Cloudberry array type, you can use the example query below:
...
> **Note** When writing JSON data, PXF supports only scalar or one dimensional arrays of Apache Cloudberry data types. PXF does not support column projection when writing JSON data.
...
Use the following [CREATE EXTERNAL TABLE](https://cloudberry.apache.org/docs/sql-stmts/create-external-table) SQL command to create a writable external table that writes JSON data in single-object-per-row format and uses the PXF default server.
...
Use the following [CREATE EXTERNAL TABLE](https://cloudberry.apache.org/docs/sql-stmts/create-external-table) SQL command to create a writable external table that writes JSON data in single object format and uses the PXF default server.
