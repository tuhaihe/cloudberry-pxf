...
description: Reading a multi-line text file as a single table row via PXF.
sidebar_position: 8
---

You can use the PXF HDFS connector to read one or more multi-line text files in HDFS each as a single table row. This may be useful when you want to read multiple files into the same Apache Cloudberry external table, for example when individual JSON files each contain a separate record.

PXF supports reading only text and JSON files in this manner.

**Note**: Refer to the [Reading and Writing JSON Data in HDFS](./hdfs_json.md) topic if you want to use PXF to read JSON files that include more than one record.


## Prerequisites

Ensure that you have met the PXF Hadoop [Prerequisites](./access_hdfs.md#prerequisites) before you attempt to read files from HDFS.
...
The following syntax creates an Apache Cloudberry readable external table that references one or more text or JSON files on HDFS:
...
The keywords and values used in the Apache Cloudberry [CREATE EXTERNAL TABLE](https://cloudberry.apache.org/docs/sql-stmts/create-external-table) command are described in the table below.
...
For example, if `/data/pxf_examples/jdir` identifies an HDFS directory that contains a number of JSON files, the following statement creates an Apache Cloudberry external table that references all of the files in that directory:
...
When you read JSON files, you can use the JSON functions provided in Apache Cloudberry to access individual data fields in the JSON record. For example, if the `pxf_readjfiles` external table above reads a JSON file that contains this JSON record:
...
Refer to [Working with JSON Data](https://cloudberry.apache.org/docs/functions/json-functions-and-operators) in the Apache Cloudberry Documentation for specific information on manipulating JSON data in Apache Cloudberry.
...
7. Log in to an Apache Cloudberry system and start the `psql` subsystem.
