...
description: Configuring the PXF JDBC connector for Hive access.
sidebar_position: 12
---
...
| `NONE`, or not specified | `TRUE` | Apache Cloudberry user name | Set `pxf.service.user.impersonation` to `true` in `jdbc-site.xml`. |
...
| `KERBEROS` | `TRUE` | Apache Cloudberry user name  | Set `pxf.service.user.impersonation` to `true` and `hadoop.security.authentication` to `kerberos` in `jdbc-site.xml`. |
...
8. If the `hive.server2.authentication` property in `hive-site.xml` is set to `NONE`, or the property is not specified, you must set the `jdbc.user` property. The value to which you set the `jdbc.user` property is dependent upon the `hive.server2.enable.doAs` impersonation setting in `hive-site.xml`:
    ...
    1. If `hive.server2.enable.doAs` is set to `TRUE` (the default), Hive runs Hadoop operations on behalf of the user connecting to Hive. *Choose/perform one of the following options*:
        ...
        **Set** `jdbc.user` to specify the user that has read permission on all Hive data accessed by Apache Cloudberry. For example, to connect to Hive and run all requests as user `gpadmin`:
        ...
        ```
        **Or**, turn on JDBC server-level user impersonation so that PXF automatically uses the Apache Cloudberry user name to connect to Hive; uncomment the `pxf.service.user.impersonation` property in `jdbc-site.xml` and set the value to `true:
    ...
    2. If required, create a PXF user configuration file as described in [Configuring a PXF User](../cfg_server.md#configuring-a-pxf-user) to manage the password setting.
...
9. If the `hive.server2.authentication` property in `hive-site.xml` is set to `KERBEROS`:
    ...
    2. Ensure that you have configured Kerberos authentication for PXF as described in [Configuring PXF for Secure HDFS](../hadoop-connectors/pxf_kerbhdfs.md), and that you have specified the Kerberos principal and keytab in the `pxf-site.xml` properties as described in the procedure.
    ...
    6. If `hive.server2.enable.doAs` is set to `TRUE` (the default), Hive runs Hadoop operations on behalf of the user connecting to Hive. *Choose/perform one of the following options*:
        ...
        **Or**, enable PXF JDBC impersonation in the `pxf-site.xml` file so that PXF automatically uses the Apache Cloudberry user name to connect to Hive. Add or uncomment the `pxf.service.user.impersonation` property and set the value to `true`. For example:
...
11. Use the `pxf cluster sync` command to copy the new server configuration to the Apache Cloudberry cluster:
