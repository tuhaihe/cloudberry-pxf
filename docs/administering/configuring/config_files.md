...
description: Reference for PXF configuration files.
sidebar_position: 3
---
...
When you update a PXF configuration file, you must synchronize the changes to all hosts in the Apache Cloudberry cluster and then restart PXF for the changes to take effect.
...
1. (Re)start PXF on all Apache Cloudberry hosts:
...
| [pxf.max.threads](../advanced-config/cfg_mem.md#another-option-for-resource-constrained-pxf-segment-hosts) | The maximum number of PXF tomcat threads. | 200 |
...
| [pxf.log.level](../advanced-config/cfg_logging.md) | The log level for the PXF Service. | info  |
...
| [server.address](../advanced-config/cfghostport.md) | The PXF server listen address. | localhost |
...
| Parameter  | Description | Default Value |
|-----------|---------------| ------------|
| [JAVA_HOME](./instcfg_pxf.md)    | The path to the Java JRE home directory. | `/usr/java/default` |
| [PXF_LOGDIR](../advanced-config/cfg_logging.md#configuring-the-log-directory)   | The PXF log directory. | `$PXF_BASE/logs` |
| PXF_RUNDIR   | The PXF run directory. | `$PXF_BASE/run` |
| [PXF_JVM_OPTS](../advanced-config/cfg_mem.md#increasing-the-jvm-memory-for-pxf)  | The default options for the PXF Java virtual machine. | `-Xmx2g -Xms1g` |
| [PXF_OOM_KILL](../advanced-config/cfg_mem.md#configuring-out-of-memory-condition-actions)  | Activate/deactivate PXF auto-termination on OutOfMemoryError (OOM). | `true (activated)` |
| [PXF_OOM_DUMP_PATH](../advanced-config/cfg_mem.md#configuring-out-of-memory-condition-actions)  | The absolute path to the dump file that PXF generates on OOM. | No dump file (empty) |
| [PXF_LOADER_PATH](../reg_jar_depend.md)  | Additional directories and JARs for PXF to class-load. | (empty) |
| [LD_LIBRARY_PATH](../reg_jar_depend.md)  | Additional directories and native libraries for PXF to load. | (empty) |
...
The [Logging](../advanced-config/cfg_logging.md) advanced configuration topic describes how to enable more verbose client-level and server-level logging for PXF.
