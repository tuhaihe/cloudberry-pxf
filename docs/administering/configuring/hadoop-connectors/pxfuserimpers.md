...
description: Configuring user impersonation and proxying for PXF.
sidebar_position: 7
---

PXF accesses Hadoop services on behalf of Apache Cloudberry end users. Impersonation is a way to present an Apache Cloudberry end user identity to a remote system. You can achieve this with PXF by configuring a Hadoop proxy user. When the Hadoop service is secured with Kerberos, you also have the option of impersonation using Kerberos constrained delegation.

When user impersonation is activated (the default), PXF accesses non-secured Hadoop services using the identity of the Apache Cloudberry user account that logs in to Apache Cloudberry and performs an operation that uses a PXF connector. Keep in mind that PXF uses only the _login_ identity of the user when accessing Hadoop services. For example, if a user logs in to Apache Cloudberry as the user `jane` and then runs `SET ROLE` or `SET SESSION AUTHORIZATION` to assume a different user identity, all PXF requests still use the identity `jane` to access Hadoop services. When user impersonation is activated, you must explicitly configure each Hadoop data source (HDFS, Hive, HBase) to allow PXF to act as a proxy for impersonating specific Hadoop users or groups.
...
If you are using PXF to access a *Kerberos-secured Hadoop cluster*, refer to the [Use Cases and Configuration Scenarios](./pxf_kerbhdfs.md#use-cases-and-configuration-scenarios) section in the *Configuring PXF for Secure HDFS* topic.

**Note**: These scenarios assume that `gpadmin` is the PXF process owner.

### Accessing Hadoop as the Apache Cloudberry User Proxied by gpadmin

This is the default configuration for PXF. The `gpadmin` user proxies Apache Cloudberry
queries on behalf of Apache Cloudberry users. The effective user in Hadoop is the
Apache Cloudberry user that runs the query.

![Accessing Hadoop as the Apache Cloudberry User Proxied by gpadmin](../../../graphics/impersonation_cases/impersonation-case-1.png "Accessing Hadoop as the Apache Cloudberry User Proxied by gpadmin")
...
|----------------|-------------------|--------------------|-----------|
|  true           |  `gpadmin`        | None; this is the default configuration. | Set the `gpadmin` user as the Hadoop proxy user as described in [Configure Hadoop Proxying](#configure-hadoop-proxying). |


### Accessing Hadoop as the Apache Cloudberry User Proxied by a &lt;custom> User

In this configuration, PXF accesses Hadoop as the Apache Cloudberry user proxied
by `<custom>` user. A query initiated by an Apache Cloudberry user appears on the Hadoop side
as originating from the `<custom>` user.

This configuration might be desirable when Hadoop is already configured with a
proxy user, or when you want a user different than `gpadmin` to proxy Apache Cloudberry queries.

![Accessing Hadoop as the Apache Cloudberry User Proxied by a custom User](../../../graphics/impersonation_cases/impersonation-case-2.png "Accessing Hadoop as the Apache Cloudberry User Proxied by a custom User")

The following table identifies the `pxf.service.user.impersonation` and `pxf.service.user.name` settings, and the PXF and Hadoop configuration required for this use case:

| Impersonation  | Service&nbsp;User |  PXF Configuration | Hadoop&nbsp;Configuration&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|----------------|-------------------|--------------------|-----------|
|  true          |  `<custom>`        | [Configure the Hadoop User](#configure-the-hadoop-user) to the `<custom>` user name. | Set the `<custom>`  user as the Hadoop proxy user as described in [Configure Hadoop Proxying](#configure-hadoop-proxying). |


### Accessing Hadoop as the gpadmin User

In this configuration, PXF accesses Hadoop as the `gpadmin` user. A query initiated by
any Apache Cloudberry user appears on the Hadoop side as originating from the `gpadmin` user.
...
| Impersonation  | Service&nbsp;User |  PXF Configuration | Hadoop&nbsp;Configuration&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|----------------|-------------------|--------------------|-----------|
|  false         |  `gpadmin`        | Turn off user impersonation as described in [Configure PXF User Impersonation](#configure-pxf-user-impersonation). | None required. |

### Accessing Hadoop as a &lt;custom> User

In this configuration, PXF accesses Hadoop as a \<custom> user. A query initiated by
any Apache Cloudberry user appears on the Hadoop side as originating from the \<custom> user.
...
| Impersonation  | Service&nbsp;User |  PXF Configuration | Hadoop&nbsp;Configuration&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |
|----------------|-------------------|--------------------|-----------|
|  false         |  \<custom>        |  Turn off user impersonation as described in [Configure PXF User Impersonation](#configure-pxf-user-impersonation) and [Configure the Hadoop User](#configure-the-hadoop-user) to the \<custom> user name. | None required. |


## Configure the Hadoop User

By default, PXF accesses Hadoop using the identity of the Apache Cloudberry user. You can configure PXF to access Hadoop as a different user on a per-server basis.
...
8. Use the `pxf cluster sync` command to synchronize the PXF Hadoop server configuration to your Apache Cloudberry cluster:
...
3. If you activated user impersonation and Kerberos constrained delegation is deactivated (the default), you must configure Hadoop proxying as described in [Configure Hadoop Proxying](#configure-hadoop-proxying). You must also configure [Hive User Impersonation](#hive-user-impersonation) and [HBase User Impersonation](#hbase-user-impersonation) if you plan to use those services.

4. Save the `pxf-site.xml` file and exit the editor.

5. Use the `pxf cluster sync` command to synchronize the PXF Hadoop server configuration to your Apache Cloudberry cluster:
...
When PXF user impersonation is activated for a Hadoop server configuration and Kerberos constrained delegation is deactivated (the default), you must configure Hadoop to permit PXF to proxy Apache Cloudberry users. This configuration involves setting certain `hadoop.proxyuser.*` properties. Follow these steps to set up PXF Hadoop proxy users:
...
5. Copy the updated `core-site.xml` file to the PXF Hadoop server configuration directory `$PXF_BASE/servers/<server_name>` on the Apache Cloudberry coordinator host and synchronize the configuration to the standby coordinator host and each Apache Cloudberry segment host.
