...
description: Monitoring the PXF service.
sidebar_position: 17
---
...
The `pxf cluster status` command displays the status of the PXF Service instance on all hosts in your Apache Cloudberry cluster. `pxf status` displays the status of the PXF Service instance on the local Apache Cloudberry host.

Only the `gpadmin` user can request the status of the PXF Service.

Perform the following procedure to request the PXF status of your Apache Cloudberry cluster.
...
If you chose the [alternate deployment topology](./deployment_topos.md#alternate-deployment-topology) for PXF, the URL is:
...
``` pre
{"build":{"version":"6.0.0","artifact":"pxf-service","name":"pxf-service","pxfApiVersion":"16","group":"org.apache.cloudberry.pxf","time":"2021-03-29T22:26:22.780Z"}}
```

To view the status of the PXF Service running on the local Apache Cloudberry host, query the `actuator/health` endpoint:
...
|---------|-------------|
| pxf.fragments.sent  | The number of fragments, and the total time that it took to send all fragments to Apache Cloudberry. |
| pxf.records.sent  | The number of records that PXF sent to Apache Cloudberry. |
| pxf.records.received  | The number of records that PXF received from Apache Cloudberry. |
| pxf.bytes.sent  | The number of bytes that PXF sent to Apache Cloudberry. |
| pxf.bytes.received  | The number of bytes that PXF received from Apache Cloudberry. |
