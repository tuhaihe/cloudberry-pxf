...
description: Registering JAR dependencies with PXF.
sidebar_position: 16
---
...
- Copy the library to the default PXF native library directory, `$PXF_BASE/lib/native`, on only the Apache Cloudberry coordinator host. When you next synchronize PXF, PXF copies the native library to all hosts in the Apache Cloudberry cluster.
- Copy the library to the default Hadoop native library directory, `/usr/lib/hadoop/lib/native`, on the Apache Cloudberry coordinator host, standby coordinator host, and each segment host.
- Copy the library to the same custom location on the Apache Cloudberry coordinator host, standby coordinator host, and each segment host, and uncomment and add the directory path to the `pxf-env.sh` `LD_LIBRARY_PATH` environment variable.


### Procedure

1. Copy the native library file to one of the following:
    - The `$PXF_BASE/lib/native` directory on the Apache Cloudberry coordinator host. (You may need to create this directory.)
    - The `/usr/lib/hadoop/lib/native` directory on all Apache Cloudberry hosts.
    - A user-defined location on all Apache Cloudberry hosts; note the file system location of the native library.
...
2. If you copied the native library to a custom location:
    ...
    2. Specify the custom location in the `LD_LIBRARY_PATH` environment variable. For example, if you copied a library named `dependent_native_lib.so` to `/usr/local/lib` on all Apache Cloudberry hosts, you would set `LD_LIBRARY_PATH` as follows:
...
3. Synchronize the PXF configuration from the Apache Cloudberry coordinator host to the standby coordinator host and segment hosts.

    ``` shell
    gpadmin@coordinator$ pxf cluster sync
    ```

    If you copied the native library to the `$PXF_BASE/lib/native` directory, this command copies the library to the same location on the Apache Cloudberry standby coordinator host and segment hosts.

    If you updated the `pxf-env.sh` `LD_LIBRARY_PATH` environment variable, this command copies the configuration change to the Apache Cloudberry standby coordinator host and segment hosts.

4. Restart PXF on all Apache Cloudberry hosts:
