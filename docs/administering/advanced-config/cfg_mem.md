...
4. Use the `pxf cluster sync` command to copy the updated `pxf-env.sh` file to the Apache Cloudberry cluster. For example:

    ``` shell
    gpadmin@coordinator$ pxf cluster sync
    ```

5. Restart PXF on each Apache Cloudberry host as described in [Restarting PXF](../cfginitstart_pxf.md#restarting-pxf).
...
By default, PXF is configured such that when the PXF JVM detects an out of memory condition on an Apache Cloudberry host, it automatically runs a script that terminates the PXF Service running on the host. The `PXF_OOM_KILL` environment variable in the `$PXF_BASE/conf/pxf-env.sh` configuration file governs this auto-terminate behavior.
...
**When the PXF Service on a host is shut down in this manner, you must explicitly restart the PXF Service on the host.** See the [pxf](../../ref/pxf.md) reference page for more information on the `pxf start` command.

Refer to the configuration [procedure](#procedure) below for the instructions to deactivate/activate this PXF configuration property.
...
Refer to the configuration [procedure](#procedure) below for the instructions to activate/deactivate this PXF configuration property.
...
6. Use the `pxf cluster sync` command to copy the updated `pxf-env.sh` file to the Apache Cloudberry cluster. For example:

    ``` shell
    gpadmin@coordinator$ pxf cluster sync
    ```

7. Restart PXF on each Apache Cloudberry host as described in [Restarting PXF](../cfginitstart_pxf.md#restarting-pxf).


## Another Option for Resource-Constrained PXF Segment Hosts

If increasing the maximum heap size is not suitable for your Apache Cloudberry deployment, try decreasing the number of concurrent working threads configured for PXF's embedded Tomcat web server. A decrease in the number of running threads will prevent any PXF server from exhausting its memory, while ensuring that current queries run to completion (albeit a bit slower). Tomcat's default behavior is to queue requests until a thread is free, or the queue is exhausted.
...
Perform the following procedure to set the maximum number of Tomcat threads for the PXF Service running on each host in your Apache Cloudberry deployment.
...
4. Use the `pxf cluster sync` command to copy the updated `pxf-application.properties` file to the Apache Cloudberry cluster. For example:
