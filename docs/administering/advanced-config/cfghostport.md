...
description: Configuring the PXF service host and port.
sidebar_position: 1
---
...
> <b>Note:</b> You must restart both Apache Cloudberry and PXF when you configure the service port number in this manner. Consider performing this configuration during a scheduled down time.

Perform the following procedure to configure the port number of the PXF server on one or more Apache Cloudberry hosts:
...
2. For each Apache Cloudberry host:

    1. Identify the port number on which you want the PXF Service to listen.
    1. Log in to the Apache Cloudberry host:
...
3. Restart Apache Cloudberry as described in [Restarting Apache Cloudberry](https://cloudberry.apache.org/docs/database-basic/start-and-stop-cbdb-database) in the Apache Cloudberry Documentation.
...
5. Verify that PXF is running on the reconfigured port by invoking `http://<PXF_HOST>:<PXF_PORT>/actuator/health` to view PXF monitoring information as described in [About PXF Service Runtime Monitoring](../monitor_pxf.md#about-pxf-service-runtime-monitoring).


## Configuring the Host

If you have chosen the [alternate deployment topology](../deployment_topos.md#alternate-deployment-topology) for PXF, you must set the `PXF_HOST` environment variable on each Apache Cloudberry segment host to inform Apache Cloudberry of the location of the PXF service. You must also set the listen address as described in [Configuring the Listen Address](#configuring-the-listen-address).

Perform the following procedure to configure the PXF host on each Apache Cloudberry segment host:

> <b>Note:</b> You must restart Apache Cloudberry when you configure the host in this manner. Consider performing this configuration during a scheduled down time.
...
2. For each Apache Cloudberry segment host:

    1. Identify the host name or IP address of a PXF Server.
    3. Log in to the Apache Cloudberry segment host:
