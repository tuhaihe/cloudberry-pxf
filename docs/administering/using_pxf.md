...
description: Granting Apache Cloudberry users access to PXF.
sidebar_position: 15
---
...
The Apache Cloudberry Platform Extension Framework (PXF) implements a protocol named `pxf` that you can use to create an external table that references data in an external data store. The PXF protocol and Java service are packaged as an Apache Cloudberry extension.
...
You must explicitly register the PXF extension in each Apache Cloudberry in which you plan to use the extension. You must have Apache Cloudberry administrator privileges to register an extension.
...
2. Create the PXF extension. You must have Apache Cloudberry administrator privileges to create an extension. For example:
...
When you no longer want to use PXF on a specific database, you must explicitly drop the PXF extension for that database. You must have Apache Cloudberry administrator privileges to drop an extension.
...
To read external data with PXF, you create an external table with the `CREATE EXTERNAL TABLE` command that specifies the `pxf` protocol. You must specifically grant `SELECT` permission to the `pxf` protocol to all non-`SUPERUSER` Apache Cloudberry roles that require such access.
