...
description: How PXF performs column projection when reading external data.
sidebar_position: 3
---
...
> **Note:** Some external data sources do not support column projection. If a query accesses a data source that does not support column projection, the query is instead run without it, and the data is filtered after it is transferred to Apache Cloudberry.
