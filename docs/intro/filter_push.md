...
description: How PXF pushes query filters down to external data sources.
sidebar_position: 2
---
...
* The Apache Cloudberry protocol that you use to access external data source must support filter pushdown. The `pxf` external table protocol supports pushdown.
...
* For queries on external tables that you create with the `pxf` protocol, the underlying PXF connector must also support filter pushdown. For example, the PXF Hive, HBase, and JDBC connectors support pushdown, as do the PXF connectors that support reading ORC and Parquet data.
  - Refer to Hive [Partition Pruning](../access-hadoop/hive_pxf.md#partition-pruning) for more information about Hive support for this feature.
