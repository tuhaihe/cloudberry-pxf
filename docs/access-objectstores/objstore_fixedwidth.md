...
description: Reading fixed-width text data in object stores via PXF.
sidebar_position: 10
---
...
The following syntax creates an Apache Cloudberry readable external table that references such a text file in an object store: 
...
1. Use the PXF `s3:fixedwidth` profile to create an Apache Cloudberry external table that references the `pxf_s3_fixedwidth.txt` file that you just created and added to S3. For example, if your server name is `s3srvcfg`:
...
This example utilizes the data schema introduced in [Example: Reading Fixed-Width Text Data on S3](#example-reading-fixed-width-text-data-on-s3). 
...
Perform the following procedure to create an Apache Cloudberry writable external table utilizing the same data schema as described above. You will use the PXF `s3:fixedwidth` profile to write data to S3. You will also create a separate, readable external table to read the data that you wrote to #3.

1. Create an Apache Cloudberry writable external table utilizing the data schema described above. Write to the S3 directory `BUCKET/pxf_examples/fixedwidth_write`. Create the table specifying `\n` as the line delimiter. For example, if your server name is `s3srvcfg`:
